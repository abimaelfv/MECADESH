	...
	class PermisosModel extends Mysql
	{
		...
		public function permisosModulo(int $idrol){
			...
			$sql = "SELECT p.rolid,
							...
							p.u,
							p.d
					FROM permisos as p JOIN modulos as m ON p.moduloid = m.idmodulo WHERE p.rolid = $this->intRolid";
			$request = $this->select($sql);
			
			
			$arrPermisosRol = array();
			for ($i=0; $i < count($request); $i++) { 
				$arrPermisosRol[$request[$i]['moduloid']] = $request[$i];
			}

			return $arrPermisosRol; // retorna a gelpers
		}
	}
 ?>